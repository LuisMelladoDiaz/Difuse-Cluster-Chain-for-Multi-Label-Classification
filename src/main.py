...
from models.LDCC_MLC import Fuzzy_LCC_MLC
...
def example_LCC_MLC():
    data_folder = 'datasets/multi_etiqueta/Emotions/emotions'
    train_file = f'{data_folder}-train.arff'
    test_file = f'{data_folder}-test.arff'
    q_labels = 6

    y_pred = LCC_MLC(train_file, q_labels)
...
def example_LDCC_MLC():
    ...
    y_pred = Fuzzy_LCC_MLC(train_file, q_labels)
