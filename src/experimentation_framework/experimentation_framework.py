...
import time
import numpy as np
from models.FCCC import compute_fuzzy_components
...
class ExperimentationFramework:
    ...
    def run_experiments(self):
        ...
        for dataset in self.datasets:
            ...
            # Compute fuzzy components once per dataset if using FCCC
            if self.model_name == "FCCC":
                # Load all data for the dataset to compute fuzzy components
                all_data = []
                for fold in range(1, self.num_folds + 1):
                    ...
                    X_train, y_train, _, _ = load_multilabel_dataset(train_file_path, self.datasets[dataset], sparse=self.sparse)
                    X_test, y_test, _, _ = load_multilabel_dataset(test_file_path, self.datasets[dataset], sparse=self.sparse)
                    all_data.append(np.vstack((y_train, y_test)))
                
                # Combine all label data
                Y_all = np.vstack(all_data)
                
                # Compute fuzzy components once
                membership_matrix, cluster_order = compute_fuzzy_components(
                    Y_all,
                    num_clusters=self.model_params.get('num_clusters', 3),
                    random_state=self.seeds[0],
                    threshold=self.model_params.get('threshold', 0.1)
                )
                
                # Add fuzzy components to model parameters
                self.model_params['membership_matrix'] = membership_matrix
                self.model_params['cluster_order'] = cluster_order
